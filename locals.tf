locals {
  ...
  planes = flatten([
    ...
    [
      for x, y in v.planes : {
        ...
        "path" : y.sub_path
        "size" : y.size
        "storage_account_name" : v.storage_type == "azurefiles" ? "${v.storage_account_name_prefix}${random_string.azurefiles.result}af" : "${v.storage_account_name_prefix}${random_string.blob.result}blob"
        ...
        "authorized_ip_ranges" : var.use_authorized_ip_ranges_only ? tomap(var.authorized_ip_ranges) : tomap(merge(var.authorized_ip_ranges, { host_ip = trimspace(data.http.host_ip[0].response_body) }))
  ...
  azurefile_storage_accounts_args = {
    for k, v in var.storage_accounts : k => merge({ storage_account_name = "${v.storage_account_name_prefix}${random_string.azurefiles.result}af" }, v, { "resource_group_name" = module.resource_groups["storage_accounts"].name }) if v.storage_type == "azurefiles"
  }

  blob_storage_accounts_args = {
    for k, v in var.storage_accounts : k => merge({ storage_account_name = "${v.storage_account_name_prefix}${random_string.blob.result}blob" }, v, { "resource_group_name" = module.resource_groups["storage_accounts"].name }) if v.storage_type == "blobnfs"
  }
