...
resource "azurerm_storage_account" "azurefiles" {
  ...
  depends_on = [random_string.azurefiles]
}
...
resource "azurerm_storage_share" "azurefiles" {
  ...
  storage_account_id = azurerm_storage_account.azurefiles[each.value.storage_account_name_prefix].id
...
resource "azurerm_storage_container" "blobnfs" {
  ...
  name                  = each.value.container_name
  storage_account_id    = azurerm_storage_account.blobnfs[each.value.storage_account_name_prefix].id
